...
```
22.9% classification rate for just creating one tree. 
...
text(ntree_values[which.max(results)],max(results), label = "(525,0.822)", pos = 4 )
```

The best value for ntree is 525.


```{r}
set.seed(1)
bag.wine <- randomForest(good ~ . - quality, data = wine, subset = train, mtry = 12, ntree = 525)
bag.wine
```
...
test error of 15.08% with bagging.
